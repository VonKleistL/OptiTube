...
final class SearchViewModel {
    ...
    var filteredItems: [SearchResultItem] {
        ...
        case .tracks:
            self.results.tracks.map { .track($0) }
        case .videos:
    ...
    @ObservationIgnored private let searchTask = TaskBox<Void, Never>()
    @ObservationIgnored private let suggestionsTask = TaskBox<Void, Never>()
    ...
    func fetchSuggestions() {
        ...
        self.suggestionsTask.replace(with: Task {
    ...
    func search() {
        ...
        self.searchTask.replace(with: Task {
    ...
    private func performSearch() async {
        ...
        do {
            ...
            case .tracks:
                try await self.client.searchTracksWithPagination(query: currentQuery)
            case .videos:
                try await self.client.searchVideosWithPagination(query: currentQuery)
            case .albums:
    ...
    func loadMore() async {
        ...
        do {
            ...
            let mergedResults = SearchResponse(
                tracks: self.results.tracks + continuation.tracks,
                videos: self.results.videos + continuation.videos,
