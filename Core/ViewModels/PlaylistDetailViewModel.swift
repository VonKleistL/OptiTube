...
final class PlaylistDetailViewModel {
    ...
    /// Strips track count patterns from author text (e.g., " • 145 tracks").
    /// Used to clean fallback author values that may contain redundant track counts.
    private func stripTrackCount(from text: String?) -> String? {
    ...
    func load() async {
        ...
        do {
            ...
            if needsMerge || thumbnailMissing {
                ...
                // Strip track counts from fallback author since we display the count separately
                let mergedPlaylist = Playlist(
                    ...
                    author: detail.author ?? self.stripTrackCount(from: self.playlist.author)
                )
                detail = PlaylistDetail(
