...
/// This class only sets up remote command handlers to route media keys to our PlaybackStore.
@MainActor
@Observable
final class NowPlayingManager {
    /// Shared singleton instance. Must be configured with `configure(playbackStore:)` before use.
    static let shared = NowPlayingManager()

    private var playbackStore: PlaybackStore?
    ...
    func configure(playbackStore: PlaybackStore) {
    ...
    private func setupRemoteCommands() {
        guard let player = playbackStore else { return }
