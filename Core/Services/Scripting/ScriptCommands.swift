...
/// AppleScript error code for when PlaybackStore is not available.
private let errPlayerNotAvailable: Int = -1728 // errAENoSuchObject

/// Error message when PlaybackStore is not available.
...
/// Checks if PlaybackStore.shared is available. Must be called from main thread.
@MainActor
private func getPlaybackStore() -> PlaybackStore? {
...
@objc(OptiTubePlayCommand)
final class PlayCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        // AppleScript commands run on the main thread
        guard let playerService = MainActor.assumeIsolated({ getPlaybackStore() }) else {
            logger.error("Play command failed: PlaybackStore.shared is nil")
...
@objc(OptiTubePauseCommand)
final class PauseCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        guard let playerService = MainActor.assumeIsolated({ getPlaybackStore() }) else {
            logger.error("Pause command failed: PlaybackStore.shared is nil")
            scriptErrorNumber = errPlayerNotAvailable
            scriptErrorString = playerNotAvailableMessage
            return nil
        }
...
@objc(OptiTubePlayPauseCommand)
final class PlayPauseCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        guard let playerService = MainActor.assumeIsolated({ getPlaybackStore() }) else {
            logger.error("PlayPause command failed: PlaybackStore.shared is nil")
            scriptErrorNumber = errPlayerNotAvailable
            scriptErrorString = playerNotAvailableMessage
            return nil
        }
...
@objc(OptiTubeNextTrackCommand)
final class NextTrackCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        guard let playerService = MainActor.assumeIsolated({ getPlaybackStore() }) else {
            logger.error("NextTrack command failed: PlaybackStore.shared is nil")
            scriptErrorNumber = errPlayerNotAvailable
            scriptErrorString = playerNotAvailableMessage
            return nil
        }
...
@objc(OptiTubePreviousTrackCommand)
final class PreviousTrackCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        guard let playerService = MainActor.assumeIsolated({ getPlaybackStore() }) else {
            logger.error("PreviousTrack command failed: PlaybackStore.shared is nil")
            scriptErrorNumber = errPlayerNotAvailable
            scriptErrorString = playerNotAvailableMessage
            return nil
        }
...
final class SetVolumeCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        ...
        guard let playerService = MainActor.assumeIsolated({ getPlaybackStore() }) else {
            logger.error("SetVolume command failed: PlaybackStore.shared is nil")
            scriptErrorNumber = errPlayerNotAvailable
            scriptErrorString = playerNotAvailableMessage
            return nil
        }
...
@objc(OptiTubeToggleShuffleCommand)
final class ToggleShuffleCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        guard let playerService = MainActor.assumeIsolated({ getPlaybackStore() }) else {
            logger.error("ToggleShuffle command failed: PlaybackStore.shared is nil")
            scriptErrorNumber = errPlayerNotAvailable
            scriptErrorString = playerNotAvailableMessage
            return nil
        }
...
/// This is a synchronous operation.
@objc(OptiTubeCycleRepeatCommand)
final class CycleRepeatCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        guard let playerService = MainActor.assumeIsolated({ getPlaybackStore() }) else {
            logger.error("CycleRepeat command failed: PlaybackStore.shared is nil")
            scriptErrorNumber = errPlayerNotAvailable
            scriptErrorString = playerNotAvailableMessage
            return nil
        }
...
@objc(OptiTubeToggleMuteCommand)
final class ToggleMuteCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        guard let playerService = MainActor.assumeIsolated({ getPlaybackStore() }) else {
            logger.error("ToggleMute command failed: PlaybackStore.shared is nil")
            scriptErrorNumber = errPlayerNotAvailable
            scriptErrorString = playerNotAvailableMessage
            return nil
        }
...
@objc(OptiTubeGetPlayerInfoCommand)
final class GetPlayerInfoCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        ...
        let result = MainActor.assumeIsolated { () -> String in
            guard let playerService = getPlaybackStore() else {
                logger.error("GetPlayerInfo command failed: PlaybackStore.shared is nil")
            ...
            if let track {
                ...
                let artworkURL = track.thumbnailURL?.absoluteString ?? ""
...
/// This is a synchronous operation.
@objc(OptiTubeLikeTrackCommand)
final class LikeTrackCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        guard let playerService = MainActor.assumeIsolated({ getPlaybackStore() }) else {
            logger.error("LikeTrack command failed: PlaybackStore.shared is nil")
            scriptErrorNumber = errPlayerNotAvailable
            scriptErrorString = playerNotAvailableMessage
            return nil
        }
...
/// This is a synchronous operation.
@objc(OptiTubeDislikeTrackCommand)
final class DislikeTrackCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        guard let playerService = MainActor.assumeIsolated({ getPlaybackStore() }) else {
            logger.error("DislikeTrack command failed: PlaybackStore.shared is nil")
            scriptErrorNumber = errPlayerNotAvailable
            scriptErrorString = playerNotAvailableMessage
            return nil
        }
