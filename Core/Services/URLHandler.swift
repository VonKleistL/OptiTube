...
///
/// Supports URLs like:
/// - `https://music.youtube.com/watch?v=dQw4w9WgXcQ` - Play track
...
/// - `optitube://play?v=dQw4w9WgXcQ` - Custom scheme for track
/// - `optitube://playlist?list=PLxxx` - Custom scheme for playlist
/// - `optitube://album?id=MPRExxx` - Custom scheme for album
/// - `optitube://artist?id=UCxxx` - Custom scheme for artist
enum URLHandler {
    // MARK: - Types

    /// Represents the type of content from a parsed URL.
    enum ParsedContent: Sendable, Equatable {
        /// A track/video to play.
    ...
    static func parse(_ url: URL) -> ParsedContent? {
        ...
        if url.scheme == "optitube" {
            return self.parseOptiTubeURL(url)
        }
    ...
    /// Parses a optitube:// custom scheme URL.
    private static func parseOptiTubeURL(_ url: URL) -> ParsedContent? {
        guard url.scheme == "optitube" else { return nil }
