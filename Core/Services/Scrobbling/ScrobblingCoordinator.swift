...
/// Bridges PlaybackStore to scrobbling backends.
/// Polls PlaybackStore at 500ms intervals (matching NotificationService pattern),
...
final class ScrobblingCoordinator {
    ...
    @ObservationIgnored private let pollingTask = TaskBox<Void, Never>()

    /// Queue flush task, cancelled in deinit.
    @ObservationIgnored private let flushTask = TaskBox<Void, Never>()

    /// Now-playing tasks, cancelled in stopMonitoring/deinit.
    @ObservationIgnored private let nowPlayingTasks = TaskCollectionBox<Void, Never>()
    ...
    /// - Parameters:
    ///   - playbackStore: The player service to monitor.
    ...
    deinit {
        ...
        self.nowPlayingTasks.cancelAll()
    }
    ...
    /// Starts monitoring PlaybackStore for scrobble-worthy events.
    ...
    private func startPolling() {
        self.pollingTask.replace(with: Task { @MainActor [weak self] in
    ...
    private func pollPlayerState() {
        ...
        let currentTrack = self.playbackStore.currentTrack
        let isPlaying = self.playbackStore.isPlaying
        let progress = self.playbackStore.progress
        let duration = self.playbackStore.duration
    ...
    private func startTrackingNewTrack(_ track: Track) {
        self.currentTrackVideoId = track.videoId
        self.trackStartTime = Date()
        self.accumulatedPlayTime = 0
        self.lastProgress = self.playbackStore.progress
    ...
    private func checkScrobbleThreshold(track: Track, duration: TimeInterval) {
    ...
    private func startQueueFlush() {
        self.flushTask.replace(with: Task { @MainActor [weak self] in
            guard let self else { return }
