...
final class LastFMService: ScrobbleServiceProtocol {
    ...
    /// Configure via `OPTITUBE_LASTFM_WORKER_URL` environment variable or Info.plist.
    ...
    @ObservationIgnored private let authPollingTask = TaskBox<Void, Never>()
    ...
    init(
        ...
        } else if let envURL = ProcessInfo.processInfo.environment["OPTITUBE_LASTFM_WORKER_URL"],
                  ...
                  let url = URL(string: bundleURL)
        {
            ...
            self.workerBaseURL = URL(string: "https://OptiTube-lastfm.VonKleistL.workers.dev")!
        }
    }
    ...
    func authenticate() async throws {
        ...
        do {
            ...
            self.authPollingTask.replace(with: Task { @MainActor [weak self] in
