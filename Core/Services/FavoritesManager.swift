...
final class FavoritesManager {
    ...
    private var fileURL: URL {
        ...
        let optitubeDir = appSupport.appendingPathComponent("OptiTube", isDirectory: true)
        return optitubeDir.appendingPathComponent("favorites.json")
    }
    ...
    /// Checks if a track is pinned.
    func isPinned(track: Track) -> Bool {
        self.isPinned(contentId: track.videoId)
    }
    ...
    /// Toggles a track in/out of Favorites.
    func toggle(track: Track) {
        self.toggle(.from(track))
    }
