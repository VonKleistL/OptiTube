...
final class LRCLibProvider: LyricsProvider {
    ...
    func search(info: LyricsSearchInfo) async -> LyricResult {
        ...
        request.setValue("OptiTube/1.0", forHTTPHeaderField: "User-Agent")
        ...
        do {
            ...
            if let synced = bestMatch.syncedLyrics, let parsed = LRCParser.parse(synced) {
                ...
                return .plain(Lyrics(lines: plain.components(separatedBy: .newlines).map { LyricLine(startTime: 0, text: $0) }, source: "Source: \(self.name)"))
            }
