...
enum PlaylistParser {
    ...
    private static func parseShelfFromSection(_ sectionData: [String: Any], key: String) -> ([Track], String?)? {
    ...
    private static func parseTracksFromContents(_ contents: [[String: Any]]) -> [Track] {
        contents.compactMap { self.parseTrackItem($0, fallbackThumbnailURL: nil) }
    }

    /// Parses liked tracks response with pagination support.
    /// Checks both legacy continuations format and 2025 continuationItemRenderer format.
    static func parseLikedTracks(_ data: [String: Any]) -> LikedTracksResponse {
        ...
        Self.logger.info("Parsed \(tracks.count) liked tracks, hasMore: \(continuationToken != nil)")
        return LikedTracksResponse(tracks: tracks, continuationToken: continuationToken)
    }

    /// Parses liked tracks continuation response.
    /// Handles both legacy musicShelfContinuation and 2025 onResponseReceivedActions formats.
    static func parseLikedTracksContinuation(_ data: [String: Any]) -> LikedTracksResponse {
        var tracks: [Track] = []
        ...
        if let continuationContents = data["continuationContents"] as? [String: Any],
           ...
           let contents = shelfContinuation["contents"] as? [[String: Any]]
        {
            Self.logger.debug("Parsing liked tracks continuation (legacy format) with \(contents.count) items")
        ...
        if tracks.isEmpty,
           ...
           let continuationItems = appendAction["continuationItems"] as? [[String: Any]]
        {
            Self.logger.debug("Parsing liked tracks continuation (2025 format) with \(continuationItems.count) items")
        ...
        Self.logger.debug("Liked tracks continuation parsed: \(tracks.count) tracks, hasMore: \(continuationToken != nil)")
        return LikedTracksResponse(tracks: tracks, continuationToken: continuationToken)
    }

    // MARK: - Continuation Token Extraction

    /// Extracts continuation token from initial browse response (liked tracks).
    /// Checks both legacy continuations format and 2025 continuationItemRenderer format.
        ...
        for sectionData in sectionContents {
            if let shelfRenderer = sectionData["musicShelfRenderer"] as? [String: Any] {
                ...
                if let token = Self.extractTokenFromRenderer(shelfRenderer) {
                    Self.logger.debug("Found liked tracks continuation token (legacy format)")
                    return token
                }
                ...
                {
                    Self.logger.debug("Found liked tracks continuation token (2025 format)")
                    return token
                }
            }
        }
    ...
    /// Extracts continuation token from a continuation response (liked tracks).
    /// Checks both legacy continuations format and 2025 continuationItemRenderer format.
    private static func extractContinuationTokenFromContinuation(_ data: [String: Any]) -> String? {
           ...
           let shelfContinuation = continuationContents["musicShelfContinuation"] as? [String: Any]
        {
            ...
            if let token = extractTokenFromRenderer(shelfContinuation) {
                self.logger.debug("Found liked tracks continuation token from continuation (legacy format)")
                return token
            }
            // Try 2025 format - continuationItemRenderer at end of contents
            ...
            {
                Self.logger.debug("Found liked tracks continuation token from continuation (2025 format)")
                return token
            }
        }
        ...
        if let onResponseReceivedActions = data["onResponseReceivedActions"] as? [[String: Any]],
           ...
           let token = Self.extractTokenFromContents(continuationItems)
        {
            Self.logger.debug("Found liked tracks continuation token from 2025 format response")
            return token
        }
    ...
    private static func parsePlaylistTracks(_ data: [String: Any], fallbackThumbnailURL: URL?) -> [Track] {
        var tracks: [Track] = []
    ...
    private static func parseTracksFromSections(_ sections: [[String: Any]], fallbackThumbnailURL: URL?) -> [Track] {
        var tracks: [Track] = []
    ...
    private static func parseTrackItem(_ data: [String: Any], fallbackThumbnailURL: URL?) -> Track? {
    ...
    private static func findTracksRecursively(in data: [String: Any], depth: Int, fallbackThumbnailURL: URL?) -> [Track] {
    ...
    static func parseQueueTracks(_ data: [String: Any]) -> [Track] {
    ...
    /// Parses a single queue item into a Track.
    private static func parseQueueItem(_ queueData: [String: Any]) -> Track? {
