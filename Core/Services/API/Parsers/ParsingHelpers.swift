...
enum ParsingHelpers {
    ...
    /// Returns the full subtitle text including track counts (e.g., "Playlist • YouTube Music • 145 tracks").
    ...
    /// Extracts track count from subtitle data (e.g., "Playlist • 145 tracks" → 145).
    static func extractTrackCountFromSubtitle(from data: [String: Any]) -> Int? {
        guard let subtitle = extractSubtitle(from: data) else { return nil }
        return Self.extractTrackCount(from: subtitle)
    }
    ...
    /// Extracts the pageType string from a browseEndpoint dict.
    ...
    static func extractArtist(from browseEndpoint: [String: Any]?, name: String, thumbnailURL: URL? = nil) -> Artist? {
        guard let browseEndpoint,
              let browseId = browseEndpoint["browseId"] as? String,
              self.isArtistPageType(self.extractPageType(from: browseEndpoint)) || Artist.isNavigableId(browseId)
        else {
            return nil
        }
    ...
    /// Extracts the first linked artist-like run from a subtitle runs array.
    static func extractFirstNavigableArtist(from runs: [[String: Any]]) -> Artist? {
        for run in runs {
            guard let name = (run["text"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines),
                  ...
                  let browseEndpoint = endpoint["browseEndpoint"] as? [String: Any],
                  let artist = self.extractArtist(from: browseEndpoint, name: name)
            else { continue }
            return artist
        }
        return nil
    ...
    static func extractDurationFromFlexColumns(_ data: [String: Any]) -> TimeInterval? {
        ...
        // Try flexColumns (artist page top tracks often have duration in the last column)
    ...
    /// Extracts track count from flex columns subtitle (e.g., "Playlist • 145 tracks" → 145).
    static func extractTrackCountFromFlexColumns(_ data: [String: Any]) -> Int? {
        guard let subtitle = extractSubtitleFromFlexColumns(data) else { return nil }
        return Self.extractTrackCount(from: subtitle)
    }

    /// Strips track count patterns from a string (e.g., " • 145 tracks" or " • 1 track").
    /// The track count is typically displayed separately in the UI from the actual parsed count.
    static func stripTrackCountPattern(from text: String) -> String {
    ...
    /// Extracts track count from subtitle text (e.g., "Playlist • YouTube Music • 145 tracks" → 145).
    static func extractTrackCount(from text: String) -> Int? {
        // Match patterns like "145 tracks" or "1 track"
        guard let regex = try? NSRegularExpression(pattern: #"(\d+)\s+tracks?"#, options: .caseInsensitive),
    ...
    private static let contentTypeKeywords: Set<String> = [
        "Track", "Video", "Album", "Playlist", "Artist", "Episode", "Podcast",
    ]
           ...
           let runs = text["runs"] as? [[String: Any]]
        {
            for run in runs {
                if let artistName = run["text"] as? String,
                   ...
                   !artistName.isEmpty,
                   // Skip content type keywords (Track, Video, etc.)
