...
enum ArtistParser { // swiftlint:disable:this type_body_length
    ...
    static func parseArtistDetail(_ data: [String: Any], artistId: String) -> ArtistDetail { // swiftlint:disable:this function_body_length cyclomatic_complexity
        ...
        var tracksBrowseId: String?
        var tracksParams: String?
           ...
           let sectionContents = sectionListRenderer["contents"] as? [[String: Any]]
        {
            for sectionData in sectionContents {
                ...
                {
                    tracksSectionTitle = tracksSectionTitle ?? ParsingHelpers.extractTitle(from: shelfRenderer)
                    buckets.tracks.append(contentsOf: self.parseTracksFromItems(shelfContents))

                    // Check if there are more tracks available via bottomEndpoint.
                    ...
                    {
                        hasMoreTracks = true
                        tracksBrowseId = browseId
                        tracksParams = browseEndpoint["params"] as? String
                ...
                // with the shelf title as a tiebreaker (Albums vs Singles & EPs).
                   ...
                   let carouselContents = carouselRenderer["contents"] as? [[String: Any]]
                {
                    ...
                    for itemData in carouselContents {
                        ...
                        switch kind {
                        case .albums:
                            if buckets.albums.count > albumCount, let album = buckets.albums.last {
                                sectionItems.albums.append(album)
                            }
                        case .singles:
                            if buckets.singles.count > singleCount, let single = buckets.singles.last {
                                sectionItems.singles.append(single)
                            }
                        case .playlistsByArtist:
                            if buckets.playlists.count > playlistCount, let playlist = buckets.playlists.last {
                                sectionItems.playlists.append(playlist)
                            }
                        case .relatedArtists:
                            if buckets.relatedArtists.count > relatedArtistCount, let artist = buckets.relatedArtists.last {
                    ...
                    // Record the shelf's "See all" endpoint against the first bucket an item landed in.
                    if let shelfKind,
        ...
        return ArtistDetail(
            ...
            songsSectionTitle: tracksSectionTitle,
    ...
    /// Extracts the shelf's "See all" browse endpoint, if present.
    /// Only returns endpoints whose `pageType` we recognise.
    ...
    // MARK: - Episodes grid (ARTIST pageType with long params)

    /// Parses a "See all episodes" response — a `gridRenderer` of `musicMultiRowListItemRenderer` items.
    ...
    /// Parses a discography browse response — a `gridRenderer` of `musicTwoRowItemRenderer` album cards.
    ...
    /// Routes a carousel item into the matching bucket. Returns the kind matched (or nil).
    @discardableResult
    private static func classifyCarouselItem(
        ...
        // Episodes / video uploads on the artist channel.
        if let multiRow = itemData["musicMultiRowListItemRenderer"] as? [String: Any],
           let episode = self.parseEpisodeFromMultiRowRenderer(multiRow)
        {
    ...
    private static func parsePlaylistFromTwoRowRenderer(
        ...
        guard let title = ParsingHelpers.extractTitle(from: data) else { return nil }
        let thumbnails = ParsingHelpers.extractThumbnails(from: data)
        let thumbnailURL = thumbnails.last.flatMap { URL(string: $0) }
        let author = ParsingHelpers.extractSubtitle(from: data)
    ...
    private static func parseCarouselItemFromTwoRowRenderer(_ data: [String: Any]) -> CarouselItem? {
           ...
           let runs = subtitleData["runs"] as? [[String: Any]]
        {
            ...
            if Self.isPlaylistBrowseId(browseId, browseEndpoint: browseEndpoint) {
                let authorName = (ParsingHelpers.extractFirstNavigableArtist(from: runs)?.name)
    ...
    private static func parseSubscriptionButton(from header: [String: Any], into result: inout HeaderParseResult) {
           ...
           let subscribeButtonRenderer = subscriptionButton["subscribeButtonRenderer"] as? [String: Any]
        {
            ...
            if let shortText = subscribeButtonRenderer["shortSubscriberCountText"] as? [String: Any],
               let runs = shortText["runs"] as? [[String: Any]]
            {
                result.subscriberCount = runs.compactMap { $0["text"] as? String }.joined()
            } else if let countText = subscribeButtonRenderer["subscriberCountText"] as? [String: Any],
                      let runs = countText["runs"] as? [[String: Any]]
            {
                result.subscriberCount = runs.compactMap { $0["text"] as? String }.joined()
            }
            if let subscribedText = subscribeButtonRenderer["subscribedButtonText"] as? [String: Any],
               let runs = subscribedText["runs"] as? [[String: Any]]
            {
                result.subscribedButtonText = runs.compactMap { $0["text"] as? String }.joined()
                    .trimmingCharacters(in: .whitespacesAndNewlines)
            }
            if let unsubscribedText = subscribeButtonRenderer["unsubscribedButtonText"] as? [String: Any],
               let runs = unsubscribedText["runs"] as? [[String: Any]]
            {
    ...
    private static func parseStartRadioButton(from header: [String: Any], into result: inout HeaderParseResult) {
        ...
        if let watchPlaylistEndpoint = navigationEndpoint["watchPlaylistEndpoint"] as? [String: Any] {
            result.mixPlaylistId = watchPlaylistEndpoint["playlistId"] as? String
            return
        }

        if let watchEndpoint = navigationEndpoint["watchEndpoint"] as? [String: Any] {
            result.mixPlaylistId = watchEndpoint["playlistId"] as? String
            result.mixVideoId = watchEndpoint["videoId"] as? String
        }
    }
    ...
    private static func extractPageType(from data: [String: Any]) -> String? {
        if let pageType = data["pageType"] as? String { return pageType }
    ...
    /// Parses tracks from artist tracks browse response.
    static func parseArtistTracks(_ data: [String: Any]) -> [Track] {
        var tracks: [Track] = []
           ...
           let sectionContents = sectionListRenderer["contents"] as? [[String: Any]]
        {
            for sectionData in sectionContents {
                if let shelfRenderer = sectionData["musicShelfRenderer"] as? [String: Any],
                   let shelfContents = shelfRenderer["contents"] as? [[String: Any]]
                {
                    tracks.append(contentsOf: self.parseTracksFromItems(shelfContents))
                }
            }
        }
           ...
           let sectionContents = sectionListRenderer["contents"] as? [[String: Any]]
        {
            for sectionData in sectionContents {
                ...
                {
                    tracks.append(contentsOf: self.parseTracksFromItems(playlistContents))
                }
            }
        }
    ...
    private static func parseTracksFromItems(_ items: [[String: Any]], fallbackThumbnailURL: URL? = nil) -> [Track] {
        var tracks: [Track] = []
