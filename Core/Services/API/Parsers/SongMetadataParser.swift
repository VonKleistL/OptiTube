...
/// Parses track metadata from YouTube Music API responses.
enum TrackMetadataParser {
    ...
    /// Parses track metadata from the "next" endpoint response.
    /// - Parameters:
    ///   - data: The response from the "next" endpoint
    ///   - videoId: The video ID of the track
    /// - Returns: A fully parsed Track object
    /// - Throws: `YTMusicError.parseError` if the structure cannot be parsed
    static func parse(_ data: [String: Any], videoId: String) throws -> Track {
    ...
    static func extractPanelVideoRenderer(from data: [String: Any], videoId: String) throws -> [String: Any] {
        guard let contents = data["contents"] as? [String: Any],
            ...
            throw YTMusicError.parseError(message: "Failed to parse track metadata for \(videoId)")
        }
