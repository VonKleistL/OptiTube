...
enum SearchResponseParser {
    ...
    static func parse(_ data: [String: Any]) -> SearchResponse {
        ...
        for sectionData in sectionContents {
            ...
            if let cardShelfRenderer = sectionData["musicCardShelfRenderer"] as? [String: Any] {
                if let item = parseCardShelfRenderer(cardShelfRenderer) {
                    Self.appendItem(item, tracks: &tracks, videos: &videos, albums: &albums, artists: &artists, playlists: &playlists)
                }
            }
        ...
        return SearchResponse(tracks: tracks, videos: videos, albums: albums, artists: artists, playlists: playlists)
    }
    ...
    /// Parses a filtered tracks-only search response.
    /// Filtered searches have a simpler structure without tabs.
    static func parseTracksOnly(_ data: [String: Any]) -> [Track] {
        var tracks: [Track] = []
    ...
    static func parseVideosOnly(_ data: [String: Any]) -> [Track] {
        var videos: [Track] = []
        ...
        guard let contents = data["contents"] as? [String: Any],
            ...
            let response = self.parse(data)
            return response.videos.isEmpty ? response.tracks : response.videos
        }
        ...
        for sectionData in sectionContents {
               ...
               let shelfContents = shelfRenderer["contents"] as? [[String: Any]]
            {
                for itemData in shelfContents {
                    if let item = parseSearchResultItem(itemData) {
                        // Whether it's parsed as .track or .video, treat it as a video
                        if case let .track(track) = item {
                            videos.append(track)
                        } else if case let .video(video) = item {
                            videos.append(video)
                        }
                    }
                }
    ...
    private static func parseTrackFromResponsiveRenderer(
        _ data: [String: Any],
        videoId: String
    ) -> SearchResultItem? {
        let thumbnails = ParsingHelpers.extractThumbnails(from: data)
    ...
    /// Parses tracks from a filtered search response with continuation token.
    static func parseTracksWithContinuation(_ data: [String: Any]) -> ([Track], String?) {
        var tracks: [Track] = []
    ...
    /// Parses videos from a filtered search response with continuation token.
    static func parseVideosWithContinuation(_ data: [String: Any]) -> ([Track], String?) {
        var videos: [Track] = []
