...
final class YTMusicClient: YTMusicClientProtocol {
    ...
    private func fetchPaginatedContent(type: PaginatedContentType, forceRefresh: Bool = false) async throws -> HomeResponse {
        self.logger.info("Fetching \(type.displayName) page (forceRefresh: \(forceRefresh))")

        let body: [String: Any] = [
            "browseId": type.rawValue,
        ]

        // If forceRefresh is true, we pass ttl: nil to bypass cache in the request method
        // but we'll still save the new result back to cache.
        let ttl = forceRefresh ? nil : APICache.TTL.home
    ...
    func getHome(forceRefresh: Bool = false) async throws -> HomeResponse {
        try await self.fetchPaginatedContent(type: .home, forceRefresh: forceRefresh)
    }
    ...
    func getPodcasts() async throws -> [PodcastSection] {
        ...
        let body: [String: Any] = [
            ...
            "params": "ggNCSgQIDBADSgQICRABSgQICBABSgQIBBABSgQIBxABSgQIDhABSgQIAxABSgQIDRABSgQIChABSgQIBhABSgQIBRAB"
        ]
    ...
    func search(query: String) async throws -> SearchResponse {
        ...
        self.logger.info("Search found \(response.tracks.count) tracks, \(response.albums.count) albums, \(response.artists.count) artists, \(response.playlists.count) playlists")
        return response
    }

    /// Searches for tracks only (filtered search).
    func searchTracks(query: String) async throws -> [Track] {
        self.logger.info("Searching tracks only for: \(query)")

        // YouTube Music API params for tracks filter
        // Derived from: EgWKAQ (filtered) + II (tracks) + AWoMEA4QChADEAQQCRAF (no spelling correction)
        let tracksFilterParams = "EgWKAQIIAWoMEA4QChADEAQQCRAF"

        let body: [String: Any] = [
            "query": query,
            "params": tracksFilterParams,
        ]

        let data = try await request("search", body: body, ttl: APICache.TTL.search)
        let tracks = SearchResponseParser.parseTracksOnly(data)
        self.logger.info("Tracks search found \(tracks.count) tracks")
        return tracks
    }

    /// Searches for videos only (filtered search).
    func searchVideos(query: String) async throws -> [Track] {
    ...
    private enum SearchFilterParams {
        static let tracks = "EgWKAQIIAWoMEA4QChADEAQQCRAF"
    ...
    /// Searches for videos only with pagination support.
    func searchVideosWithPagination(query: String) async throws -> SearchResponse {
        self.logger.info("Searching videos with pagination for: \(query)")
        ...
        let data = try await request("search", body: body, ttl: APICache.TTL.search)
        let (videos, token) = SearchResponseParser.parseVideosWithContinuation(data)
        self.searchContinuationToken = token

        self.logger.info("Videos search found \(videos.count) videos, hasMore: \(token != nil)")
        return SearchResponse(tracks: [], videos: videos, albums: [], artists: [], playlists: [], continuationToken: token)
    }
    ...
    func searchAlbums(query: String) async throws -> SearchResponse {
        ...
        return SearchResponse(tracks: [], albums: albums, artists: [], playlists: [], continuationToken: token)
    }
    ...
    func searchArtists(query: String) async throws -> SearchResponse {
        ...
        return SearchResponse(tracks: [], albums: [], artists: artists, playlists: [], continuationToken: token)
    }
    ...
    func searchPlaylists(query: String) async throws -> SearchResponse {
        ...
        return SearchResponse(tracks: [], albums: [], artists: [], playlists: playlists, continuationToken: token)
    }
    ...
    /// Searches for tracks only with pagination support.
    func searchTracksWithPagination(query: String) async throws -> SearchResponse {
        self.logger.info("Searching tracks with pagination for: \(query)")

        let body: [String: Any] = [
            "query": query,
            "params": SearchFilterParams.tracks,
        ]

        let data = try await request("search", body: body, ttl: APICache.TTL.search)
        let (tracks, token) = SearchResponseParser.parseTracksWithContinuation(data)
        self.searchContinuationToken = token

        self.logger.info("Tracks search found \(tracks.count) tracks, hasMore: \(token != nil)")
        return SearchResponse(tracks: tracks, albums: [], artists: [], playlists: [], continuationToken: token)
    }
    ...
    func resetSessionStateForAccountSwitch() {
        ...
        self.searchContinuationToken = nil
        self.likedTracksContinuationToken = nil
    ...
    // MARK: - Liked Tracks with Pagination

    /// Continuation token for liked tracks pagination.
    private var likedTracksContinuationToken: String?

    /// Whether more liked tracks are available to load.
    var hasMoreLikedTracks: Bool {
        self.likedTracksContinuationToken != nil
    }

    /// Fetches the user's liked tracks with pagination support.
    /// Uses VLLM (Liked Music playlist) which returns all tracks with proper pagination,
    /// unlike FEmusic_liked_videos which is limited to ~13 tracks.
    func getLikedTracks() async throws -> LikedTracksResponse {
        self.logger.info("Fetching liked tracks via VLLM playlist")
        ...
        self.likedTracksContinuationToken = playlistResponse.continuationToken
        let hasMore = playlistResponse.hasMore

        // Convert to LikedTracksResponse format
        let response = LikedTracksResponse(
            tracks: playlistResponse.detail.tracks,
            continuationToken: playlistResponse.continuationToken
        )

        self.logger.info("Parsed \(response.tracks.count) liked tracks, hasMore: \(hasMore)")
        return response
    }

    /// Fetches the next batch of liked tracks via continuation.
    /// Returns nil if no more tracks are available.
    func getLikedTracksContinuation() async throws -> LikedTracksResponse? {
        guard let token = likedTracksContinuationToken else {
            self.logger.debug("No liked tracks continuation token available")
            return nil
        }

        self.logger.info("Fetching liked tracks continuation")
        ...
        do {
            ...
            self.logger.info("Liked tracks continuation loaded: \(response.tracks.count) tracks, hasMore: \(hasMore)")
            return response
        } catch {
            self.logger.warning("Failed to fetch liked tracks continuation: \(error.localizedDescription)")
            self.likedTracksContinuationToken = nil
            throw error
        }
    }
    ...
    /// Fetches artist details including their tracks and albums.
    func getArtist(id: String) async throws -> ArtistDetail {
        ...
        self.logger.info("Parsed artist '\(detail.artist.name)' with \(detail.tracks.count) tracks and \(detail.albums.count) albums")
        return detail
    }

    /// Fetches all tracks for an artist using the tracks browse endpoint.
    func getArtistTracks(browseId: String, params: String?) async throws -> [Track] {
        self.logger.info("Fetching artist tracks: \(browseId)")
        ...
        self.logger.info("Parsed \(tracks.count) artist tracks")
        return tracks
    }

    // MARK: - Lyrics

    /// Fetches lyrics for a track by video ID.
    ...
    func getRadioQueue(videoId: String) async throws -> [Track] {
        ...
        self.logger.info("Fetched radio queue with \(result.tracks.count) tracks")
    ...
    /// - Returns: RadioQueueResult with tracks and continuation token for infinite mix
    func getMixQueue(playlistId: String, startVideoId: String?) async throws -> RadioQueueResult {
        ...
        let data = try await request("next", body: body)
        let result = RadioQueueParser.parse(from: data)
        self.logger.info("Fetched mix queue with \(result.tracks.count) tracks, hasContinuation: \(result.continuationToken != nil)")
        return result
    }

    /// Fetches more tracks for a mix queue using a continuation token.
    /// - Parameter continuationToken: The continuation token from a previous getMixQueue call
    /// - Returns: RadioQueueResult with additional tracks and next continuation token
    func getMixQueueContinuation(continuationToken: String) async throws -> RadioQueueResult {
        ...
        self.logger.info("Fetched \(result.tracks.count) more tracks, hasContinuation: \(result.continuationToken != nil)")
        return result
    }

    // MARK: - Track Metadata

    /// Fetches full track metadata including feedbackTokens for library management.
    /// Uses the `next` endpoint to get track details with library status.
    /// - Parameter videoId: The video ID of the track
    /// - Returns: A Track with full metadata including feedbackTokens and inLibrary status
    func getTrack(videoId: String) async throws -> Track {
        self.logger.info("Fetching track metadata: \(videoId)")
        ...
        let data = try await request("next", body: body, ttl: APICache.TTL.trackMetadata)
        let track = try TrackMetadataParser.parse(data, videoId: videoId)
        self.logger.info("Parsed track '\(track.title)' - inLibrary: \(track.isInLibrary ?? false), hasTokens: \(track.feedbackTokens != nil)")
        return track
    }
    ...
    /// These are browse pages that return sections of tracks/playlists, not playlist tracks.
    /// - Parameters:
    ...
    /// Switches the active identity to the specified brand account.
    /// This updates the internal state so subsequent requests use the correct identity.
    ...
    /// Rates a track (like/dislike/indifferent).
    /// - Parameters:
    ///   - videoId: The video ID of the track to rate
    ...
    func rateTrack(videoId: String, rating: LikeStatus) async throws {
        self.logger.info("Rating track \(videoId) with \(rating.rawValue)")
        ...
        self.logger.info("Successfully rated track \(videoId)")

        // Invalidate mutation-affected caches in a single pass
        APICache.shared.invalidateMutationCaches()
    }

    /// Adds or removes a track from the user's library.
    /// - Parameter feedbackTokens: Tokens obtained from track metadata (use add token to add, remove token to remove)
    func editTrackLibraryStatus(feedbackTokens: [String]) async throws {
        ...
        self.logger.info("Editing track library status with \(feedbackTokens.count) tokens")
