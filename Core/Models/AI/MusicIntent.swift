...
/// The model generates this from free-form text like "play some jazz" or "skip this track".
...
struct MusicIntent: Sendable {
    ...
    /// Search query or track/artist name (for play, queue, search actions).
    @Guide(description: "The search query, track title, or artist name. Empty for actions like skip, pause, resume.")
    let query: String

    /// Scope for shuffle action (e.g., "library", "playlist", "artist").
    @Guide(description: "The scope for shuffle: all, library, likes, or empty for single track actions.")
    ...
    func buildSearchQuery() -> String {
        // If we have a specific query already (like a track title), use it
        ...
        // Append "tracks" suffix if we don't have "hits" or "music" already
    ...
    private func buildGenericQuery() -> [String] {
        ...
        // "chill music" returns better playlists than "chill tracks"
    ...
    private func cleanQueryForAppending(_ query: String) -> String {
        ...
        let skipWords: Set<String> = [
            ...
            "tracks", "music", "tracks", "hits", "hit", "best", "greatest", "top",
        ]
