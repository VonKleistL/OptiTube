...
struct FavoriteItem: Identifiable, Codable, Sendable {
    ...
    /// Creates a FavoriteItem from a Track.
    static func from(_ track: Track) -> FavoriteItem {
        FavoriteItem(itemType: .track(track))
    }
    ...
    /// Uses videoId for tracks, browseId for others.
