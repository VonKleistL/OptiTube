...
enum ArtistDetailSectionContent: Hashable, Sendable {
    case albums([Album])
...
struct ArtistDetailSection: Identifiable, Hashable, Sendable {
    let title: String
...
/// Contains detailed artist information including their tracks.
struct ArtistDetail: Sendable {
    ...
    /// Whether there are more tracks available beyond what's loaded.
    let hasMoreTracks: Bool
    /// Browse ID for loading all tracks (e.g., from "More tracks" button).
    ...
    init(
        ...
        tracksBrowseId: String? = nil,
        tracksParams: String? = nil,
        ...
        self.tracksBrowseId = tracksBrowseId
        self.tracksParams = tracksParams
