...
/// Represents a track/track from YouTube Music.
struct Track: Identifiable, Codable, Hashable, Sendable {
    ...
    /// Like/dislike status of the track (nil if unknown).
    var likeStatus: LikeStatus?

    /// Whether the track is in the user's library (nil if unknown).
...
extension Track {
    /// Creates a Track from YouTube Music API response data.
    init?(from data: [String: Any]) {
        ...
        if let durationSeconds = data["duration_seconds"] as? Double {
            ...
            self.duration = Track.parseDuration(durationString)
        } else {
            self.duration = nil
        }
