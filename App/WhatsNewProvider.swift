...
enum WhatsNewProvider {
    ...
    static let fallbackCollection: [WhatsNew] = [
        WhatsNew(
            ...
            features: [
                ...
                .init(
                    icon: "waveform",
                    title: "Native Equalizer",
                    subtitle: "6-band parametric EQ powered by Audio Process Tap"
                ),
                .init(
                    icon: "text.bubble.fill",
                    title: "Synced Lyrics",
                    subtitle: "Follow along with time-synced lyrics and romanization"
                ),
                .init(
            ...
            learnMoreURL: URL(string: "https://github.com/VonKleistL/OptiTube/releases")
        ),
    ]
    ...
    static func parseRelease(data: Data) -> WhatsNew? {
        ...
        let title = name ?? "What's New in OptiTube \(versionString)"
    ...
    static func cleanReleaseBody(_ body: String) -> String {
    ...
    private static func extractSection(from lines: [String], preferredHeadings: Set<String>) -> [String]? {
        guard let startIndex = lines.firstIndex(where: { line in
            guard let heading = Self.markdownHeading(in: line) else { return false }
        ...
        while index < lines.count {
            ...
            if let hiddenSectionLevel {
                if let heading, heading.level <= hiddenSectionLevel {
                    // Resume at next sibling heading
                } else {
                    index += 1
                    continue
                }
