...
final class AppDelegate: NSObject, NSApplicationDelegate {
    /// Reference to the PlaybackStore for dock menu actions.
    /// Set by OptiTubeApp after initialization.
    ...
    func applicationDidFinishLaunching(_: Notification) {
        ...
        // Restore saved queue/session if available.
        self.playbackStore?.restoreQueueFromPersistence()
    }
    ...
    @objc private func systemWillSleep(_: Notification) {
        ...
        self.wasPlayingBeforeSleep = self.playbackStore?.isPlaying ?? false
    ...
    private func setupWindowDelegate() {
        for window in NSApplication.shared.windows where window.canBecomeMain {
            ...
            if window.frameAutosaveName.isEmpty {
                window.setFrameAutosaveName("OptiTubeMainWindow")
            }
            // True clear glass window — the NSVisualEffectView layers inside the
            // view hierarchy provide the behind-window blur. Setting the window
            // itself to transparent removes the opaque fallback that produces the
            // milky/foggy appearance.
    ...
    @objc private func dockMenuPlayPause() {
        guard let playbackStore else {
            // Fallback to direct WebView control if PlaybackStore not available
            SingletonPlayerWebView.shared.playPause()
            return
        }
        Task {
            await playbackStore.playPause()
        }
    }
