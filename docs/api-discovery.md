...
> **Complete documentation of YouTube Music API endpoints for OptiTube development.**
>
...
### Regular YouTube Video Mode

Verified 2026-07-04 with `api-explorer.swift --youtube`:

- Home recommendations: `browse` with `FEwhat_to_watch`.
- Home can return only `feedNudgeRenderer` and no continuation when YouTube has insufficient watch history; do not synthesize recommendations.
- Explore topics: web initial data from `/feed/explore`; `FEexplore` and `FEtrending` return HTTP 400 with the current `WEB` client.
- Search: `search` action with `{ "query": "..." }`.
- Pagination responses use either `onResponseReceivedActions` or `onResponseReceivedCommands`, each containing `appendContinuationItemsAction`.
- Large continuation tokens can be piped without exposing them as arguments: `... | ./Tools/api-explorer.swift continuation - search --youtube`.
...
When browser cookies exist but no longer represent a valid server session, this endpoint can return HTTP 200 with only `responseContext` and `selectText`; `actions` is absent. Treat an empty parsed account list as expired authentication, not a valid zero-account result.
...
| `FEmusic_charts` | Charts |  | Top tracks, albums by country/genre | `HomeResponseParser` |
...
| `VLLM` | Liked Tracks |  | All tracks user has liked (with pagination) | `PlaylistParser` |
| `VL{playlistId}` | Playlist Detail |  | Playlist tracks and metadata | `PlaylistParser` |
| `UC{channelId}` | Artist Detail |  | Artist page with tracks, albums | `ArtistParser` |
| `MPLYt{id}` | Lyrics |  | Track lyrics text | `LyricsParser` |
...
> **Note**: Charts, Moods & Genres, and New Releases all use `HomeResponseParser` since they share the same section-based response structure. `VLLM` is a special case of `VL{playlistId}` where `LM` is the Liked Music playlist ID. Do NOT use `FEmusic_liked_videos` — it returns only ~13 tracks without pagination.
...
>  **Use `VLLM`, not `FEmusic_liked_videos`** — The `FEmusic_liked_videos` browse ID returns only ~13 tracks with NO continuation token. To fetch all liked tracks, use `VLLM` (VL prefix + LM playlist ID) which returns the full list with proper pagination.

```swift
//  Correct: Use VLLM for all liked tracks
let body = ["browseId": "VLLM"]
// Requires authentication

//  Avoid: FEmusic_liked_videos is limited to ~13 tracks
// let body = ["browseId": "FEmusic_liked_videos"]
```

**Returns**: Playlist-format response with all liked tracks and continuation token for pagination
...
These endpoints are functional but not yet implemented in OptiTube.
...
| `FEmusic_library_tracks` | Library Tracks |  | Low | Requires auth + params* |
...
| `FEmusic_library_privately_owned_tracks` | Uploaded Tracks |  | Low | Uploaded tracks |
| `FEmusic_library_privately_owned_albums` | Uploaded Albums |  | Low | Uploaded albums |

> \* Library Albums/Artists/Tracks return HTTP 400 without authentication. With authentication, they also require specific `params` values for sorting. The exact param encoding needs to be captured from web client requests.
...
- Contains filter chips for: Playlists, Podcasts, Tracks, Albums, Artists, Profiles
...
>  **Critical**: The suffix already starts with `L`. Adding `"PL"` instead of `"P"` creates a double-L (`PLLXz2p9...`) which causes HTTP 404 errors. Always use `"P" + suffix`, never `"PL" + suffix`.
...
|----------|------|------|-------------|
| `search` | Search |  | Search tracks, albums, artists, playlists |
| `music/get_search_suggestions` | Suggestions |  | Autocomplete for search |
| `next` | Now Playing |  | Track info, lyrics ID, radio queue |
| `like/like` | Like |  | Like a track/album/playlist |
| `like/dislike` | Dislike |  | Dislike a track |
...
**Response Structure**:
- `musicCardShelfRenderer` — **Top Result** section (single prominent result: track, album, artist, or playlist)
- `musicShelfRenderer` — Regular results (mixed tracks, albums, artists, playlists)

>  **Important**: The Top Result (most relevant match) is returned in `musicCardShelfRenderer`, not `musicShelfRenderer`. This is often the artist/album the user is looking for. Always parse both renderer types.
...
```json
{
  "musicCardShelfRenderer": {
    ...
    "thumbnail": { ... },
    "contents": [ /* related tracks/albums */ ]
  }
}
```
...
|--------|-------------|-------------|
| Tracks | `EgWKAQIIAWoMEA4QChADEAQQCRAF` | Filter to tracks only |
...
> **Filter Pattern**: `EgWKAQ` (base) + filter code + `AWoMEA4QChADEAQQCRAF` (no spelling correction suffix). The filter code encodes the content type (tracks=II, albums=IY, artists=Ig, playlists=Io, podcasts=JQ).
...
- `getTrack(videoId:)` - Gets full track metadata with tokens
...
```swift
// Add to library using token from track metadata
let body = ["feedbackTokens": [addToken]]
_ = try await request("feedback", body: body)
```

Tokens come from `getTrack(videoId:)` response.
...
//
//  IMPORTANT: Do NOT add "PL" prefix - that would create "PLLXz2p9..." which returns 404!
...
>  **Note**: Podcast subscription uses `like/like` and `like/removelike` endpoints, NOT `subscription/*`. The MPSPP browse ID must be converted to a PL playlist ID by stripping "MPSPP" and prepending "P" (not "PL").
...
>  **Note**: The response uses a **wrapper structure** (`playlistPanelVideoWrapperRenderer.primaryRenderer.playlistPanelVideoRenderer`)
...
| `musicShelfRenderer` | Vertical list (search results, artist tracks) |
| `musicTwoRowItemRenderer` | Album/playlist card |
| `musicResponsiveListItemRenderer` | Track row |
...
| `SearchResponseParser` | `SearchResponseParser.swift` | Search response | `SearchResponse` with tracks, albums, artists, playlists | `search` endpoint |
...
| `ArtistParser` | `ArtistParser.swift` | Artist browse response | `ArtistDetail` with tracks, albums | `UC{channelId}` |
...
| `TrackMetadataParser` | `TrackMetadataParser.swift` | Next endpoint response | `Track` with full metadata | `next` endpoint |
| `RadioQueueParser` | `RadioQueueParser.swift` | Next endpoint response | `RadioQueueResult` with tracks + continuation | Radio/mix playback |
...
ParsingHelpers.extractText(from: titleRuns)  // -> "Track Title"
...
OptiTube uses a unified `YTMusicError` enum for all API-related errors. This enables consistent error handling, user-friendly messages, and retry logic.
...
For authenticated endpoints (), sign in to the OptiTube app first:
...
Debug builds export auth cookies for the API explorer to `~/Library/Application Support/OptiTube/cookies.dat`.
...
| 2024-12-21 | Confirmed Library Albums/Artists/Tracks require auth + params |
...
| User Generated Content | `MUSIC_VIDEO_TYPE_UGC` | Fan-made or unofficial |  Varies |
...
- [Core/Models/Track.swift](../Core/Models/Track.swift) - `musicVideoType` property
- [Core/Services/API/Parsers/TrackMetadataParser.swift](../Core/Services/API/Parsers/TrackMetadataParser.swift) - Parsing logic
...
if track.musicVideoType?.hasVideoContent == true {
    showVideoToggle()
}
```
...
>  **Not Implemented**: Due to DRM requirements, OptiTube uses WebView for playback. Direct URL streaming would bypass DRM protection. Quality selection would need to be implemented via WebView JavaScript.
...
The `next` endpoint returns a Related tab that can find track/video counterparts.

>  **Not Implemented**: Could be used to find video version of audio-only tracks or vice versa.

**Related Tab browseId Pattern**: `MPTRt_{trackId}`

**Example**: For track `DyDfgMOUjCI`, the Related tab browseId is `MPTRt_5OAD9vk2OaS`
...
###  Works with Session Cookies (from visiting music.youtube.com)
...
| `FEmusic_library_tracks` | HTTP 400 | Needs auth + specific `params` value |
...
> **Note on Library Albums/Artists/Tracks**: These endpoints return HTTP 400 even with session cookies. They require both full SAPISIDHASH authentication AND a specific `params` value (protobuf-encoded sorting options). The exact params need to be captured from the web client's network requests.
