...
3. **Create Container**: Creates `#optitube-video-container` with `position: fixed`
...
1. `window.__optitubeTargetVolume` stores the app's target volume
...
**Solution**: Inject a blackout overlay (`#optitube-blackout`) immediately when entering video mode. This black `<div>` covers the entire viewport until video extraction is complete.

### Issue: Volume Wrong on Track Start (FIXED)

**Root Cause**: The WKUserScript for volume initialization was created at WebView creation time, capturing a stale volume value. When pages reloaded for new tracks, this stale script ran.
...
3. Observer script applies `__optitubeTargetVolume` when video element is first detected

### Issue: Video Availability

**Root Cause**: Not all tracks have music videos available.

**Solution**: `PlaybackStore.currentTrackHasVideo` tracks video availability using a hybrid detection approach. The Video button is only enabled when a video exists.
...
When `fetchTrackMetadata` is called, the parser extracts `musicVideoType` from the YouTube Music API response:
...
- [Core/Services/API/Parsers/TrackMetadataParser.swift](../Core/Services/API/Parsers/TrackMetadataParser.swift) — `parseMusicVideoType(from:)` method
- [Core/Services/Player/PlaybackStore+Library.swift](../Core/Services/Player/PlaybackStore+Library.swift) — Calls `updateVideoAvailability(hasVideo:)` after parsing
...
```javascript
// Detects Track/Video toggle button
...
Track starts playing
    ...
    │   └── PlaybackStore.updateVideoAvailability() called (fast, may be inaccurate)
    │
    └── fetchTrackMetadata() completes
        └── musicVideoType parsed
            └── PlaybackStore.updateVideoAvailability() called (authoritative)
```
