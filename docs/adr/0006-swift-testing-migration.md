...
**Files to create:**
- `Tests/OptiTubeTests/SwiftTestingHelpers/Tags.swift` — Define custom test tags
...
1. **Split `OptiTubeTests.swift`** before migration:
   - Move `testTrackDurationParsing`, `testTrackDurationDisplayWithNoDuration` → `ModelTests.swift`
   ...
   - Delete `OptiTubeTests.swift` once empty
...
**Files in `OptiTubeUITests/`:**
- `OptiTubeUITestCase.swift` (base class)
...
| Task | Status |
|------|--------|
| Split `OptiTubeTests.swift` into proper test files |  Done |
| Delete `OptiTubeTests.swift` after split |  Done |
| Verify no mocks use `static var` |  Done |

### Unit Tests (`OptiTubeTests/`)
...
| `PlaybackStoreTests.swift` | 5 | High |  Done |
...
```swift
// Tests/OptiTubeTests/SwiftTestingHelpers/Tags.swift
import Testing
...
1. **Create `Tags.swift`** in `Tests/OptiTubeTests/SwiftTestingHelpers/`
2. **Split `OptiTubeTests.swift`** into proper test files
3. **Migrate `LikeStatusTests.swift`** as the pilot (simplest file)
4. **Verify both frameworks run together**: `xcodebuild test -only-testing:OptiTubeTests`
