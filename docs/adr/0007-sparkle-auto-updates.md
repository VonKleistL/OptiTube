...
OptiTube is distributed outside the Mac App Store via GitHub Releases and Homebrew Cask. Users need a reliable way to receive updates without manually downloading new versions. The lack of automatic updates creates friction for users and delays security fixes and feature rollouts.
...
3. **Manual check** (OptiTube → Check for Updates...):
...
```xml
<key>SUFeedURL</key>
<string>https://raw.githubusercontent.com/VonKleistL/OptiTube/main/appcast.xml</string>
...
```bash
./Tools/sign-update.sh ./build/OptiTube-v1.2.3.dmg
```
