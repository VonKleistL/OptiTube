...
This document provides detailed information about OptiTube's architecture, services, and design patterns.
...
Core/               → Shared logic (platform-independent)
  ├── Models/       → Data types (Track, Playlist, Album, Artist, etc.)
  ├── Services/     → Business logic
  │   ├── API/      → YTMusicClient, Parsers/
  │   ├── Auth/     → AuthService (login state machine)
  │   ├── Player/   → PlaybackStore, NowPlayingManager (media keys)
...
protocol PlaybackStoreProtocol { ... }
```
...
- `getLikedTracks()` → User's liked tracks (with pagination via `getLikedTracksContinuation()`)
...
- `getArtist(id:)` → Artist details with tracks and albums
- `getLyrics(videoId:)` → Lyrics for a track (two-step: next → browse)
- `rateTrack(videoId:rating:)` → Like/dislike a track
...
| `ArtistParser.swift` | Artist details (tracks, albums, subscription status) |
| `RadioQueueParser.swift` | Radio queue from "next" endpoint |
| `TrackMetadataParser.swift` | Full track metadata with feedback tokens |
...
**File**: `Core/Services/Player/PlaybackStore.swift`
...
- `play(track:)` — Plays a Track model
...
final class SingletonPlayerWebView {
    ...
    func getWebView(webKitManager:, playbackStore:) -> WKWebView
...
- Routes commands to `PlaybackStore` → `SingletonPlayerWebView`
...
**Supported Item Types**: Track, Album, Playlist, Artist
...
**Persistence**:
- **Location**: `~/Library/Application Support/OptiTube/favorites.json`
...
- Observes `PlaybackStore.currentTrack` for changes
...
**Conforming Types**: `Track`, `Playlist`, `Album`, `Artist`

**Share Text Format**: "Title by Artist" or just "Title" for artists.

### TrackLikeStatusManager

**File**: `Core/Services/TrackLikeStatusManager.swift`

Caches and syncs like/dislike status for tracks:

| Method | Description |
|--------|-------------|
| `status(for:)` | Get cached status for video ID or Track |
| `isLiked(_:)` | Check if track is liked |
...
Parses YouTube Music and custom `optitube://` URLs:

| URL Pattern | ParsedContent |
|-------------|---------------|
| `music.youtube.com/watch?v=xxx` | `.track(videoId:)` |
...
| `optitube://play?v=xxx` | `.track(videoId:)` |
| `optitube://playlist?list=xxx` | `.playlist(id:)` |
| `optitube://album?id=xxx` | `.album(id:)` |
| `optitube://artist?id=xxx` | `.artist(id:)` |

**Usage**: Called from `OptiTubeApp.onOpenURL` to handle deep links.
...
- Commands access `PlaybackStore.shared` via `MainActor.assumeIsolated` (AppleScript runs on main thread)
- Synchronous methods (shuffle, repeat, like/dislike) execute directly
- Async methods (play, pause, volume) spawn detached Tasks
- All commands return AppleScript errors (`-1728`) if `PlaybackStore.shared` is nil
- Logging via `DiagnosticsLogger.scripting`

**Related Files**:
- `App/OptiTube.sdef` — AppleScript dictionary definition
- `App/Info.plist` — `NSAppleScriptEnabled` and `OSAScriptingDefinition` keys

**Usage**:
```applescript
tell application "OptiTube"
    play
    set volume 50
...
│  → PlaybackStore updates isPlaying, progress    │
└─────────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────────┐
...
│ NowPlayingManager                               │
│  → Registers media key handlers → PlaybackStore │
└─────────────────────────────────────────────────┘
```
...
| 2 minutes | Search |
| 30 minutes | Playlist, Track metadata |
| 1 hour | Artist |
| 24 hours | Lyrics |
...
OptiTube integrates Apple's on-device Foundation Models framework for AI-powered features. See [ADR-0005: Foundation Models Architecture](adr/0005-foundation-models-architecture.md) for detailed design decisions.
...
┌─────────────────────────────────────────────────┐
│ Execute Action                                  │
│  → MusicIntent → PlaybackStore                  │
│  → QueueIntent → PlaybackStore queue methods    │
...
Searches the YouTube Music catalog for tracks, artists, and albums:
...
- Enables commands like "remove duplicates" or "play upbeat tracks next"
...
**Integration**: Toggled via `PlaybackStore.showQueue`, placed outside `NavigationSplitView` in `MainWindow`.

### LyricsView

**File**: `Views/macOS/LyricsView.swift`

Right sidebar panel displaying track lyrics:
...
**Integration**: Toggled via `PlaybackStore.showLyrics`, persists across all navigation states.
...
| `TrackActionsHelper` | Common track action handlers |
...
- Remove placeholder items (Artists, Albums, Tracks, Liked Tracks, etc.)
...
var mainContent: some View {
    HStack(spacing: 0) {
        ...
        LyricsView(...)
            .frame(width: playbackStore.showLyrics ? 280 : 0)
    }
}
```
...
All UI components require macOS 15.0+ for Liquid Glass:
