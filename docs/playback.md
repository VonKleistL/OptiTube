...
| `PlaybackStore` | `PlaybackStore.swift` | Playback state and control |
...
```swift
// In a view
playbackStore.play(videoId: "dQw4w9WgXcQ")
```
...
if let videoId = playbackStore.pendingPlayVideoId {
    PersistentPlayerView(videoId: videoId, isExpanded: playbackStore.showMiniPlayer)
...
Swift receives and updates `PlaybackStore`:
...
```swift
.frame(
    width: playbackStore.showMiniPlayer ? 160 : 1,
    height: playbackStore.showMiniPlayer ? 90 : 1
)
```
...
```swift
// In Coordinator
if isPlaying && playbackStore.showMiniPlayer {
...
1. **Initial Load**: `playWithMix()` fetches ~50 tracks via the `next` endpoint
2. **Continuation Token**: The API returns a `nextRadioContinuationData.continuation` token
3. **Auto-Fetch**: When ≤10 tracks remain in queue, `fetchMoreMixTracksIfNeeded()` loads more
4. **Duplicate Filter**: New tracks are filtered to prevent duplicates
...
| Component | Purpose |
|-----------|----------|
| `RadioQueueResult` | Holds tracks + continuation token |
| `mixContinuationToken` | Stored in `PlaybackStore` |
| `isFetchingMoreMixTracks` | Prevents concurrent fetches |
| `fetchMoreMixTracksIfNeeded()` | Triggered on `next()` and `playFromQueue()` |

### State Reset

The continuation token is cleared when:
- Playing a regular queue (`playQueue`)
- Playing track radio (`playWithRadio`)
