...
//  Reads cookies from the OptiTube app's debug cookie export for authenticated requests.
//
//  Usage:
...
/// Reads cookies from OptiTube app's backup file in Application Support.
...
func loadCookiesFromAppBackup() -> [HTTPCookie]? {
    ...
    guard let data = try? Data(contentsOf: cookieFile) else {
        print(" Cookie file exists but failed to read: \(cookieFile.path)")
        return nil
    }
    ...
    guard let cookieDataArray = try? NSKeyedUnarchiver.unarchivedObject(
        ...
        print(" Cookie file exists but failed to unarchive. File may be corrupted or use a different format.")
...
func filterCookiesForMusicYouTube(_ cookies: [HTTPCookie]) -> [HTTPCookie] {
    let targetDomain = isYouTubeMode ? "www.youtube.com" : "music.youtube.com"
    return cookies.filter { cookie in
        let domain = cookie.domain.lowercased()
        if domain.hasPrefix(".") {
            let withoutDot = String(domain.dropFirst())
            return targetDomain.hasSuffix(withoutDot) || withoutDot == targetDomain
        }
        return domain == targetDomain || targetDomain.hasSuffix("." + domain)
    }
}
...
func analyzeResponse(_ data: [String: Any], verbose: Bool = false) -> String {
    ...
    if let header = data["header"] as? [String: Any] {
        output += "\n Header keys: \(header.keys.sorted().joined(separator: ", "))\n"
    }
...
func exploreBrowse(_ browseId: String, params: String? = nil, verbose: Bool = false, outputFile: String? = nil) async {
    ...
    do {
        ...
        if statusCode == 401 || statusCode == 403 {
            ...
            print("   Run the OptiTube app and sign in, then try again.")
            return
        }
            ...
            for (key, value) in continuationContents {
                if let renderer = value as? [String: Any] {
                    if let contents = renderer["contents"] as? [[String: Any]] {
                        ...
                        // For playlistPanelContinuation (mix queues), show track count
                        if key == "playlistPanelContinuation" {
                            ...
                            print("   └─ Tracks in continuation: \(trackCount)")
                        }
                    }
                    if let continuations = renderer["continuations"] as? [[String: Any]] {
                        ...
                        {
                            print("   └─ Has nextRadioContinuationData (more mix tracks available)")
                        }
                    }
                }
            }
        } else if let actions = (data["onResponseReceivedActions"] as? [[String: Any]]) ??
            (data["onResponseReceivedCommands"] as? [[String: Any]])
        {
            print("   Found action/command continuation format")
            ...
            print("    No recognized continuation format found")
...
func checkAuthStatus() {
    ...
    guard let cookies = loadCookiesFromAppBackup() else {
        ...
        print("  1. Run the OptiTube app")
        print("  2. Sign in to YouTube Music")
        print("  3. The app will save cookies to ~/Library/Application Support/OptiTube/")
...
func discoverAccounts(verbose: Bool) async {
    ...
    guard loadCookiesFromAppBackup() != nil else {
        print(" No cookies found. Please sign in to OptiTube first.")
        return
    }
...
func listEndpoints() {
    ...
    FEmusic_charts                Top tracks, albums, trending by country/genre
    ...
    FEmusic_liked_videos          Liked tracks (returns playlist format)
    ...
    FEmusic_library_tracks         All tracks in library (requires params*)
    ...
    FEmusic_library_privately_owned_tracks    User-uploaded tracks
    ...
    ───────────────────────────────────────────────────────────────────────────────
    like/like                     Like a track/album/playlist
                                  Body: {"target": {"videoId": "VIDEO_ID"}}

    like/dislike                  Dislike a track
                                  Body: {"target": {"videoId": "VIDEO_ID"}}
    ...
    ═══════════════════════════════════════════════════════════════════════════════
     LIBRARY PARAMS (for library_albums, library_artists, library_tracks)
    ═══════════════════════════════════════════════════════════════════════════════
...
func showHelp() {
    ...
    Supports both public and authenticated endpoints (reads cookies from OptiTube app).
    ...
    Commands:
      browse <browseId> [params]     Explore a browse endpoint
      action <endpoint> <body>       Explore an action endpoint (body as JSON)
      continuation <token|-> [ep]    Explore a continuation; '-' reads token from stdin
      list                           List all known endpoints
      auth                           Check authentication status
      ...
      ./api-explorer.swift auth
        ...
        Authentication:
            ...
            Debug builds export auth cookies to:
                ~/Library/Application Support/OptiTube/cookies.dat
...
func fetchLiveYouTubeConfig() async -> (apiKey: String, clientVersion: String)? {
    ...
    if let cookies = loadCookiesFromAppBackup() {
        if let cookieHeader = buildCookieHeader(from: cookies) {
    ...
    do {
        ...
        guard let versionMatch = versionRegex.firstMatch(in: html, options: [], range: versionRange),
              let r2 = Range(versionMatch.range(at: 1), in: html) else {
            return nil
        }
        let extractedVersion = String(html[r2])

        return (extractedKey, extractedVersion)
    } catch {
        print(" Failed to fetch live YouTube config: \(error.localizedDescription)")
        return nil
    }
}
...
func runMain() async {
    ...
    case "continuation":
        ...
        let token: String
        if filteredArgs[1] == "-" {
            guard let stdin = String(data: FileHandle.standardInput.readDataToEndOfFile(), encoding: .utf8)?
                .trimmingCharacters(in: .whitespacesAndNewlines),
                !stdin.isEmpty
            else {
                print(" No continuation token received on stdin")
                return
            }
            token = stdin
        } else {
            token = filteredArgs[1]
        }
