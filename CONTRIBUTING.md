...
Or open `OptiTube.xcodeproj` in Xcode and press ⌘R.
...
2. **Build Must Pass** — Run `xcodebuild -scheme OptiTube -destination 'platform=macOS' build`
3. **Tests Must Pass** — Run `xcodebuild -scheme OptiTube -destination 'platform=macOS' test`
...
- Add unit test in PlaybackStoreTests.swift
...
xcodebuild -scheme OptiTube -destination 'platform=macOS' \
  test -only-testing:OptiTubeTests/PlaybackStoreTests
```
