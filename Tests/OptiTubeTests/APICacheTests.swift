...
struct APICacheTests {
    ...
    func cacheTTLConstants() {
        ...
        #expect(APICache.TTL.trackMetadata == 30 * 60) // 30 minutes
    }
    ...
    func lyricsCacheNotInvalidatedByMutations() {
        ...
        self.cache.set(key: "next:track_abc123", data: ["title": "track"], ttl: APICache.TTL.trackMetadata)

        self.cache.invalidate(matching: "next:")

        #expect(self.cache.get(key: "browse:lyrics_abc123") != nil)
        #expect(self.cache.get(key: "next:track_abc123") == nil)
    }

    @Test("Track metadata cache invalidated by mutations")
    func trackMetadataCacheInvalidatedByMutations() {
        self.cache.set(key: "next:track_abc123", data: ["title": "track"], ttl: APICache.TTL.trackMetadata)
