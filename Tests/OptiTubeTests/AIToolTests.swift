...
struct MusicSearchToolTests {
    ...
    @Test("Search returns formatted track results")
    func searchReturnsTracks() async throws {
        self.mockClient.searchResponse = TestFixtures.makeSearchResponse(
            trackCount: 3, albumCount: 0, artistCount: 0, playlistCount: 0
        )

        let args = MusicSearchTool.Arguments(query: "jazz", filter: "tracks")
    ...
    func searchReturnsAlbums() async throws {
        self.mockClient.searchResponse = TestFixtures.makeSearchResponse(
            trackCount: 0, albumCount: 2, artistCount: 0, playlistCount: 0
        )
    ...
    func searchReturnsArtists() async throws {
        self.mockClient.searchResponse = TestFixtures.makeSearchResponse(
            trackCount: 0, albumCount: 0, artistCount: 2, playlistCount: 0
        )
    ...
    func searchReturnsPlaylists() async throws {
        self.mockClient.searchResponse = TestFixtures.makeSearchResponse(
            trackCount: 0, albumCount: 0, artistCount: 0, playlistCount: 2
        )
    ...
    func searchAllFilterReturnsMixed() async throws {
        self.mockClient.searchResponse = TestFixtures.makeSearchResponse(
            trackCount: 2, albumCount: 1, artistCount: 1, playlistCount: 1
        )
    ...
    func searchPropagatesErrors() async throws {
        ...
        let args = MusicSearchTool.Arguments(query: "test", filter: "tracks")
...
struct QueueToolTests {
    ...
    func toolMetadata() {
        ...
        let tool = QueueTool(playbackStore: playbackStore)
