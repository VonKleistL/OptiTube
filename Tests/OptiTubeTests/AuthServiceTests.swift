...
struct AuthServiceTests {
    ...
    @Test("Expired credential cannot immediately complete login again")
    func expiredCredentialIsRejected() {
        self.authService.completeLogin(sapisid: "expired-sapisid")
        self.authService.sessionExpired()

        #expect(self.authService.completeLogin(sapisid: "expired-sapisid") == false)
        #expect(self.authService.state == .loggedOut)
        #expect(self.authService.completeLogin(sapisid: "new-sapisid") == true)
        #expect(self.authService.state == .loggedIn(sapisid: "new-sapisid"))
    }
    ...
    @Test("checkLoginStatus waits for initial cookie restore")
    func checkLoginStatusWaitsForRestore() async {
        self.mockWebKitManager.sapisidValue = nil
    ...
    @Test("Empty account response requires reauthentication")
    func emptyAccountResponseRequiresReauthentication() async {
        self.authService.completeLogin(sapisid: "test-sapisid")
        let accountService = AccountService(
            ytMusicClient: MockYTMusicClient(),
        ...
        #expect(self.authService.state == .loggedOut)
        #expect(self.authService.needsReauth == true)
        #expect(accountService.isLoading == false)
        #expect(accountService.lastError is YTMusicError)
    }
