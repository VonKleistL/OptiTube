...
///   -only-testing:OptiTubeTests/ParserPerformanceTests
/// ```
final class ParserPerformanceTests: XCTestCase {
    ...
    func testSearchParsingPerformance() {
        ...
        let data = self.makeSearchResponseData(tracks: 20, albums: 10, artists: 10, playlists: 10)
    ...
    func testSearchTracksOnlyPerformance() {
        // Given: Search results with many tracks
        let data = self.makeSearchResponseData(tracks: 100, albums: 0, artists: 0, playlists: 0)

        // When/Then: Measure track-heavy parsing
        measure {
            _ = SearchResponseParser.parse(data)
        }
    }
    ...
    func testArtistDetailParsingPerformance() {
        // Given: An artist with many tracks and albums
        let data = self.makeArtistDetailData(trackCount: 50, albumCount: 20)
    ...
    private func makeArtistDetailData(trackCount: Int, albumCount: Int) -> [String: Any] {
        var trackItems: [[String: Any]] = []
        for i in 0 ..< trackCount {
            trackItems.append([
                "musicResponsiveListItemRenderer": [
                    "playlistItemData": ["videoId": "video\(i)"],
