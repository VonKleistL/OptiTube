...
// MARK: - Track + CustomTestStringConvertible

/// Provides readable test descriptions for Track in test failure logs.
///
/// When a test fails involving a Track, Swift Testing will display:
/// `"Never Gonna Give You Up" by Rick Astley (3:33)` instead of the full debug description.
extension Track: @retroactive CustomTestStringConvertible {
...
/// `"My Playlist" (25 tracks)` instead of the full debug description.
extension Playlist: @retroactive CustomTestStringConvertible {
    public var testDescription: String {
...
extension Album: @retroactive CustomTestStringConvertible {
    public var testDescription: String {
...
extension Artist: @retroactive CustomTestStringConvertible {
    public var testDescription: String {
        "Artist: \"\(name)\""
    }
}
...
extension PlaylistDetail: @retroactive CustomTestStringConvertible {
    public var testDescription: String {
        let typeText = isAlbum ? "album" : "playlist"
...
extension HomeSectionItem: @retroactive CustomTestStringConvertible {
    public var testDescription: String {
        switch self {
        case let .track(track):
            "Track: \(track.testDescription)"
...
extension PodcastShow: @retroactive CustomTestStringConvertible {
    public var testDescription: String {
...
extension SearchResultItem: @retroactive CustomTestStringConvertible {
    public var testDescription: String {
        switch self {
        case let .track(track):
            "SearchResult[Track]: \(track.testDescription)"
        case let .album(album):
