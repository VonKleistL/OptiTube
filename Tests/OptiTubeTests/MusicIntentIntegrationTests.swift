...
struct MusicIntentIntegrationTests {
    ...
    - query: The raw search text or track/artist name
    ...
    @Test("Parses playback control commands", arguments: [
        ...
        (prompt: "Skip this track", expectedAction: MusicAction.skip),
        ...
        (prompt: "Like this track", expectedAction: MusicAction.like),
    ...
    @Test("Parses artist queries", arguments: [
        ...
        (prompt: "Play Taylor Swift tracks", expected: "taylor"),
    ])
    ...
    @Test("Parses activity-based queries", arguments: [
        ...
        (prompt: "Play workout tracks", expected: "workout"),
    ])
    ...
    func parsePreviousCommand() async throws {
        try await self.withRetry {
            try await self.parseIntent(from: "Go back to the previous track")
        } validate: { intent in
    ...
    func parseDislikeCommand() async throws {
        try await self.withRetry {
            try await self.parseIntent(from: "I don't like this track")
        } validate: { intent in
    ...
    func parseArtistWithMood() async throws {
        try await self.withRetry {
            try await self.parseIntent(from: "Play energetic tracks by Daft Punk")
        } validate: { intent in
            #expect(intent.action == .play)
