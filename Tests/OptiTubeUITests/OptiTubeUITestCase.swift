...
struct MockFavoriteItem {
    ...
    enum MockFavoriteType {
        case track(videoId: String, title: String, artist: String)
    ...
    /// Creates a mock track favorite.
    static func track(videoId: String, title: String, artist: String) -> MockFavoriteItem {
        MockFavoriteItem(type: .track(videoId: videoId, title: title, artist: artist))
    }
...
// MARK: - OptiTubeUITestCase

/// Base class for OptiTube UI tests.
/// Provides common setup, launch configuration, and helper methods.
@MainActor
class OptiTubeUITestCase: XCTestCase {
        ...
        let sections = (0 ..< sectionCount).map { sectionIndex in
            [
                ...
                "items": (0 ..< itemsPerSection).map { itemIndex in
                    [
                        "type": "track",
                        "id": "track-\(sectionIndex)-\(itemIndex)",
                        "title": "Track \(itemIndex)",
    ...
    func launchWithMockSearch(trackCount: Int = 5) {
        ...
        if let jsonData = try? JSONSerialization.data(withJSONObject: ["tracks": tracks]),
           let jsonString = String(data: jsonData, encoding: .utf8)
        {
    ...
    func launchWithMockFavorites(_ items: [MockFavoriteItem]) {
        let favorites = items.map { item -> [String: Any] in
            ...
            switch item.type {
            case let .track(videoId, title, artist):
                dict["itemType"] = [
                    "track": [
                        "_0": [
                            "id": videoId,
