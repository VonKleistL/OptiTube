...
struct ExtensionOptionsView: NSViewRepresentable {
    ...
    func updateNSView(_ webView: WKWebView, context _: Context) {
        if webView.url != self.page.url, !webView.isLoading {
            // Tiny delay to ensure WebKit's internal extension registry has caught up with the ID
            Task { @MainActor in
    ...
    static func dismantleNSView(_ nsView: WKWebView, coordinator: Coordinator) {
        nsView.configuration.userContentController.removeScriptMessageHandler(forName: "optionsDebug")
    }
