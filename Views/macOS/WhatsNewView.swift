...
struct WhatsNewView: View {
    ...
    private var contentView: some View {
        if let releaseNotes = self.whatsNew.releaseNotes {
            WhatsNewMarkdownView(markdown: releaseNotes)
...
struct WhatsNewMarkdownView: View {
    ...
    private var blocks: [AnyView] {
        ...
        while i < lines.count {
            ...
            if trimmed.isEmpty {
                ...
                flushParagraph()
                repeat { i += 1 } while i < lines.count && !lines[i - 1].contains("-->")
            } else if Self.isSpacerLine(trimmed) || Self.isThematicBreak(trimmed) {
                flushParagraph()
                i += 1
            } else if trimmed.hasPrefix("### ") {
                flushParagraph()
                result.append(AnyView(Self.heading(Self.inlineMarkdown(String(trimmed.dropFirst(4))), font: .headline, topPadding: 2)))
                i += 1
            } else if trimmed.hasPrefix("## ") {
                flushParagraph()
                result.append(AnyView(Self.heading(Self.inlineMarkdown(String(trimmed.dropFirst(3))), font: .title3.weight(.bold), topPadding: 6)))
                i += 1
            } else if trimmed.hasPrefix("# ") {
                flushParagraph()
                result.append(AnyView(Self.heading(Self.inlineMarkdown(String(trimmed.dropFirst(2))), font: .title2.weight(.bold), topPadding: 10)))
                i += 1
                ...
                while i < lines.count {
                    ...
                    guard let item = Self.listItemText(from: listLine) else { break }
                    items.append(item)
                    i += 1
                }
                ...
                result.append(AnyView(Self.codeBlock(codeLines.joined(separator: "\n"))))
            } else {
    ...
    private static func isSpacerLine(_ line: String) -> Bool {
        ["<br>", "<br/>", "<br />"].contains(line.lowercased())
    }
