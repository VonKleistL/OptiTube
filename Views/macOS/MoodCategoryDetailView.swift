...
/// Displays sections of tracks and playlists for the selected mood/genre.
...
struct MoodCategoryDetailView: View {
        ...
        Group {
            if self.viewModel.sections.isEmpty {
                ContentUnavailableView(
                    ...
                    description: Text("No tracks or playlists found in this category.")
                )
            } else {
                ScrollView {
    ...
    /// Tracks use Button (play action), other items use NavigationLink.
    @ViewBuilder
    private func itemView(_ item: HomeSectionItem) -> some View {
        switch item {
        case let .track(track):
            // Tracks play directly
