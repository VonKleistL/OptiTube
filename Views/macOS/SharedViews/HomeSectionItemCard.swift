...
/// Reusable card view for home section items (tracks, playlists, albums, artists).
...
struct HomeSectionItemCard: View {
    ...
    private var thumbnail: some View {
        ...
        .overlay {
            // Play overlay on hover (for tracks)
            if case .track = self.item, self.isHovering {
                Circle()
...
#Preview {
    let track = Track(
        id: "test",
        title: "Test Track with a Very Long Title That Should Wrap",
        artists: [Artist(id: "artist1", name: "Test Artist")],
        videoId: "testVideo"
    )
    HStack {
        HomeSectionItemCard(item: .track(track)) {
            // No-op for preview
        }
        HomeSectionItemCard(item: .track(track), rank: 1) {
            // No-op for preview
        }
    }
    .padding()
