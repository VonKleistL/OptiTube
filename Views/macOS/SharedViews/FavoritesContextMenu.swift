...
enum FavoritesContextMenu {
    /// Creates a context menu button for toggling a track in/out of Favorites.
    @ViewBuilder
    static func menuItem(for track: Track, manager: FavoritesManager) -> some View {
        let isPinned = manager.isPinned(track: track)
        Button {
            manager.toggle(track: track)
        } label: {
            Label(
    ...
    static func menuItem(for item: HomeSectionItem, manager: FavoritesManager) -> some View {
        ...
        case let .track(track):
            Self.menuItem(for: track, manager: manager)
    ...
    static func menuItem(for item: SearchResultItem, manager: FavoritesManager) -> some View {
        ...
        case let .video(video):
            Self.menuItem(for: video, manager: manager)
        case let .album(album):
            Self.menuItem(for: album, manager: manager)
        case let .playlist(playlist):
            Self.menuItem(for: playlist, manager: manager)
