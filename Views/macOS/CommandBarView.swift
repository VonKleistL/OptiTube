...
struct CommandBarView: View {
    ...
    private var suggestionsHeaderText: String {
        ...
        } else if !self.playbackStore.queue.isEmpty {
            "Your queue is ready:"
        } else {
            "Try commands like:"
        }
    ...
    private var discoverySuggestions: [String] {
        if self.playbackStore.currentTrack != nil {
            ...
            ["Add jazz to queue", "Skip this track", "What's in my queue?"]
        } else if !self.playbackStore.queue.isEmpty {
            // Queue exists - offer to modify it
            ["Add more tracks", "Play something different", "Start over"]
        } else {
    ...
    private var aiSystemInstructions: String {
        """
        You are a music assistant for the OptiTube app. Parse the user's natural language command
        ...
        Example: "upbeat rolling stones tracks from the 90s"
        → action: play, query: "upbeat rolling stones tracks", artist: "Rolling Stones", mood: "upbeat", era: "1990s"
    ...
    private func processCommand() async {
        ...
        let queueTool = QueueTool(playbackStore: self.playbackStore)
    ...
    private func fallbackDirectSearch(query: String) async {
        ...
        if lowered.hasPrefix("play ") {
            ...
            if lowered.contains("queue") {
                ...
                let status = self.playbackStore.shuffleEnabled ? "on" : "off"
    ...
    private func extractMoodOrGenre(from query: String) -> String {
        ...
        let moodPatterns = ["something", "some", "anything", "music that", "tracks that"]
    ...
    private func extractSimilarArtist(from query: String) -> String? {
        let patterns = [
            ...
            "artists like ", "tracks like ", "music like ",
        ]
    ...
    private func executeIntent(_ intent: MusicIntent) async {
        ...
        case .play:
            ...
            if searchQuery.isEmpty, intent.query.isEmpty {
                await self.playbackStore.resume()
        ...
        case .pause:
            HapticService.playback()
            await self.playbackStore.pause()
    ...
    private func playSearchResult(query: String, description: String = "") async {
        do {
            // Use tracks-only filtered search to exclude podcasts/videos
            let allTracks = try await client.searchTracks(query: query)
            let tracks = Array(allTracks.prefix(20))

            self.logger.info("Tracks search returned \(allTracks.count), using top \(tracks.count) for query: \(query)")
            if let firstTrack = tracks.first {
                // Use playQueue to populate the queue with search results
                await self.playbackStore.playQueue(tracks, startingAt: 0)
                self.logger.info("Started queue with \(tracks.count) tracks, first: \(firstTrack.title)")
                // Use description if available for nicer feedback
                if !description.isEmpty {
                    self.resultMessage = "Playing \(description)"
                } else {
                    self.resultMessage = "Playing \"\(firstTrack.title)\""
                }
            } else {
                self.errorMessage = "No tracks found for \"\(query)\""
            }
        } catch {
    ...
    private func queueSearchResult(query: String, description: String = "") async {
        do {
            // Use tracks-only filtered search to exclude podcasts/videos
            let allTracks = try await client.searchTracks(query: query)
            let tracks = Array(allTracks.prefix(10))

            self.logger.info("Queue tracks search returned \(allTracks.count), using top \(tracks.count) for query: \(query)")
            if let firstTrack = tracks.first {
                if self.playbackStore.queue.isEmpty {
                    ...
                    if !description.isEmpty {
                        ...
                        self.resultMessage = "Playing \"\(firstTrack.title)\" and \(tracks.count - 1) more"
                    }
                } else {
                    // Add all tracks to existing queue
                    self.playbackStore.appendToQueue(tracks)
                    if !description.isEmpty {
                        self.resultMessage = "Added \(description) to queue"
                    } else {
                        self.resultMessage = "Added \(tracks.count) tracks to queue"
                    }
                }
            } else {
                self.errorMessage = "No tracks found for \"\(query)\""
    ...
    private func playContent(intent: MusicIntent, query: String, description: String, source: ContentSource) async {
        ...
        case .moodsAndGenres:
            ...
            if let tracks = await self.findTracksFromMoodsAndGenres(intent: intent) {
                await self.playbackStore.playQueue(tracks, startingAt: 0)
        ...
        case .charts:
            // Try to get tracks from Charts
            if let tracks = await self.findTracksFromCharts() {
                await self.playbackStore.playQueue(tracks, startingAt: 0)
                self.resultMessage = "Playing top tracks"
            } else {
                // Fallback to search
                await self.playSearchResult(query: query, description: description)
            }
        ...
        case .moodsAndGenres:
            if let tracks = await self.findTracksFromMoodsAndGenres(intent: intent) {
                if self.playbackStore.queue.isEmpty {
                    ...
                    self.playbackStore.appendToQueue(tracks)
                    self.resultMessage = "Added \(description.isEmpty ? "curated tracks" : description) to queue"
                }
            } else {
        ...
        case .charts:
            if let tracks = await self.findTracksFromCharts() {
                if self.playbackStore.queue.isEmpty {
                    ...
                    self.playbackStore.appendToQueue(tracks)
                    self.resultMessage = "Added top tracks to queue"
                }
            } else {
                await self.queueSearchResult(query: query, description: description)
            }
    ...
    /// Finds tracks from Moods & Genres that match the intent.
    private func findTracksFromMoodsAndGenres(intent: MusicIntent) async -> [Track]? {
        do {
            ...
            for section in response.sections {
                ...
                if self.matchesSearchTerms(section.title, searchTerms) {
                    ...
                    {
                        return try await self.fetchPlaylistTracks(playlistId: playlist.id)
                    }
                    // If section has tracks directly, use those
                    let tracks = section.items.compactMap { item -> Track? in
                        if case let .track(track) = item { return track }
                        return nil
                    }
                    if !tracks.isEmpty {
                        return Array(tracks.prefix(25))
                    }
                }
    ...
    /// Finds top tracks from Charts.
    private func findTracksFromCharts() async -> [Track]? {
        do {
            let response = try await client.getCharts()

            // Look for track sections in charts
            for section in response.sections {
                let tracks = section.items.compactMap { item -> Track? in
                    if case let .track(track) = item { return track }
                    return nil
    ...
    /// Fetches tracks from a playlist by ID.
    private func fetchPlaylistTracks(playlistId: String) async throws -> [Track] {
