        ...
        NavigationStack(path: self.$navigationPath) {
            Group {
                if !self.networkMonitor.isConnected {
                    ...
                    case .idle, .loading:
                        LoadingView(L("Loading moods & genres..."))
            ...
            .navigationTitle(L("Moods & Genres"))
        ...
        Group {
            if self.viewModel.sections.isEmpty {
                ContentUnavailableView(
                    L("No Moods & Genres Available"),
                    systemImage: "guitars",
                    description: Text(L("Content may not be available in your region."))
                )
            } else {
                ScrollView {
