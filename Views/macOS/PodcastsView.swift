...
struct PodcastsView: View {
    ...
    private func playEpisode(_ episode: PodcastEpisode) {
        // Create a Track from the episode to play via WebView
        let track = Track(
            id: episode.id,
            title: episode.title,
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                // Progress bar (Apple Podcasts style)
                    ProgressView(value: self.episode.playbackProgress)
                        .tint(self.episode.isPlayed ? .secondary : .appAccent)
            }
            .frame(width: 200)
        }
        .buttonStyle(.plain)
...
struct PodcastShowView: View {
    ...
    private func playEpisodeInQueue(at index: Int) {
        let tracks = self.episodes.map { self.episodeToTrack($0) }
        Task {
            await self.playbackStore.playQueue(tracks, startingAt: index)
        }
    }

    /// Converts a podcast episode to a Track for playback.
    private func episodeToTrack(_ episode: PodcastEpisode) -> Track {
        Track(
            id: episode.id,
            title: episode.title,
            artists: episode.showTitle.map { [Artist(id: "podcast", name: $0)] } ?? [],
    ...
    private func toggleSubscription() async {
        ...
        do {
            if self.isSubscribed {
                try await TrackActionsHelper.unsubscribeFromPodcast(
                    self.show,
                    client: self.client,
                ...
                try await TrackActionsHelper.subscribeToPodcast(
                    self.show,
                    client: self.client,
                    libraryViewModel: self.libraryViewModel
                )
