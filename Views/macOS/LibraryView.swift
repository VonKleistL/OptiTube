...
struct LibraryView: View {
        ...
        NavigationStack(path: self.$navigationPath) {
            Group {
                if !self.networkMonitor.isConnected {
                    ...
                    case .idle, .loading:
                        LoadingView(L("Loading your library..."))
    ...
    private var emptyStateMessage: String {
        switch self.selectedFilter {
        case .all:
            L("Save playlists and subscribe to podcasts on YouTube Music to see them here.")
        case .playlists:
            L("Create or save playlists on YouTube Music to see them here.")
        case .podcasts:
            L("Subscribe to podcasts on YouTube Music to see them here.")
        }
    }
        ...
        Button {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                if let count = playlist.trackCount {
                    Text(LF("%lld tracks", Int64(count)))
                        .font(.system(size: 11))
                        .foregroundStyle(.secondary)
                }
            }
