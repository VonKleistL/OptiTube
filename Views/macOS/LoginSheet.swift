...
struct LoginSheet: View {
    ...
    private var headerView: some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack {
                Text(L("Sign in to YouTube Music"))
                    .font(.headline)
            ...
            Text(L("Note: If passkeys don't work, use \"Try another way\" to sign in with password."))
                .font(.caption)
    ...
    private func checkForSuccessfulLoginAsync() async {
        ...
        if let sapisid = await webKitManager.getSAPISID() {
            ...
            guard self.authService.completeLogin(sapisid: sapisid) else {
