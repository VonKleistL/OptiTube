...
struct SearchView: View {
    ...
    private var searchField: some View {
        HStack {
            ...
            TextField(L("Search tracks, albums, artists..."), text: self.$viewModel.query)
    ...
    private var contentView: some View {
        if !self.networkMonitor.isConnected {
            ...
            case .loading, .loadingMore:
                LoadingView(L("Searching..."))
            case .loaded, .authRequired:
    ...
    private var emptyStateView: some View {
        VStack(spacing: 16) {
            ...
            Text(self.viewModel.query.isEmpty ? L("Search for your favorite music") : L("Press Enter to search"))
                .font(.title3)
                .foregroundStyle(.secondary)

            Text(L("Find tracks, albums, artists, and playlists"))
                .font(.subheadline)
                .foregroundStyle(.tertiary)
        }
    ...
    private var noResultsView: some View {
        VStack(spacing: 16) {
            ...
            Text(L("Try searching for something else"))
                .font(.subheadline)
                .foregroundStyle(.tertiary)
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity)
    ...
    private func contextMenuItems(for item: SearchResultItem) -> some View {
        switch item {
        case let .track(track), let .video(track):
            Button {
        ...
        case let .podcastShow(show):
            Button {
                ...
                Label(L("View Podcast"), systemImage: "mic.fill")
            }
